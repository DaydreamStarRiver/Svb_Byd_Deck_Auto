...
1. 出牌选项
   ...
   模式2：执行第二个选项的坐标点击操作
   点击敌方玩家：直接选择敌方玩家作为目标
   点击敌方双随从：选择敌方血量最高的两个随从作为目标
   点击护盾或最高血量：优先选择有护盾的随从，没有则选择血量最高的敌方随从
   点击血量小于6的敌方随从：选择血量小于6的敌方随从，没有则选择血量最高的敌方随从

2. 进化选项
   作用：为卡牌选择进化时对应的操作模式
   空选项：不会把这张卡认作进化时出牌选项卡，将以普通卡牌处理
   模式1 (进化)：进化时执行第一个选项的坐标点击操作
   模式2 (进化)：进化时执行第二个选项的坐标点击操作
   点击血量小于4的敌方随从：进化时选择血量小于4的敌方随从
   点击血量最高的敌方随从：进化时选择血量最高的敌方随从


三、特殊操作逻辑

1. 目标选择
   - 对于需要选择敌方目标的卡牌，脚本会先扫描敌方战场信息，再选择最合适的目标
...
建议及后续更新：
   ...
   3. 后续如果会有空，会添加更多特殊操作选项和智能目标选择逻辑
"""
        # 创建消息框
